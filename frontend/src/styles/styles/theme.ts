...
import { clamp } from 'styles/utils/clamp'
...
export function colorVariablesScale(color: string, name: string, increment = 5) {
...
export function modifyColor(base: string, extra: { h?: number, s?: number, l?: number }){
  ...
  saturation = clamp(saturation + ((s || 0)/100 || 0), 0, 1)
  lightness = clamp(lightness + ((l || 0)/100 || 0), 0, 1)

  return hslToColorString({ hue, saturation, lightness })
}

export function getPropColor(key: string, props: {[key:string]:any}) {
